...
#define S5P64XX_EXT_INT1_FLTCON0		(S5P64XX_VA_GPIO + 0x0E88)
...
static int __devinit onedram_probe(struct platform_device *pdev)
{
	...
	temp |= ((0x1 << 7) |(0x1 << 6)|(0x3f));// bit7 : enable, bit6 : digital filter, bit[5:0] filter clock count
	writel(readl(S5P64XX_EXT_INT1_FLTCON0) |temp ,S5P64XX_EXT_INT1_FLTCON0);
...
//	struct onedram *od = platform_get_drvdata(pdev);
	int temp;
	// SET FILTER
	temp = readl(S5P64XX_EXT_INT1_FLTCON0);
	temp &= ~(0xff); // clear bit [7:0]
	temp |= (0x1 << 7); // bit7 : enable, bit6 : digital filter, bit[5:0] filter clock count
	writel(temp , S5P64XX_EXT_INT1_FLTCON0);
	return 0;
}
